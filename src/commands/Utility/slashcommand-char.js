...
async function handleDMResponse(
    client,
    interaction,
    dmChannel,
    member,
...
async function handleDMError(client, interaction, member, error, lang) {
    await interaction.editReply({
        content: LanguageManager.getText(
            'commands.global_strings.dm_failed',
            lang,
            {
                username: member.user.tag,
            }
...
function createCharacterListEmbed(userData, targetUser, lang) {
    const embed = new EmbedBuilder()
        .setColor(0x0099ff)
        .setTitle(
            LanguageManager.getText('commands.charlist.embed.title', lang, {
    ...
    if (
        !userData ||
    (!userData.main && (!userData.alts || userData.alts.length === 0))
    ) {
        embed.setDescription(
            LanguageManager.getText('commands.charlist.embed.no_characters', lang)
        );
    } else {
        let description = '';
        ...
        LanguageManager.getText(
            'commands.charlist.embed.main_character',
            lang,
            {
        ...
        if (userData.alts && userData.alts.length > 0) {
            description +=
        LanguageManager.getText(
            'commands.charlist.embed.alt_characters_header',
            lang
        ) + '\n';
            userData.alts.forEach((alt) => {
                description +=
          LanguageManager.getText(
              'commands.charlist.embed.character_entry',
              lang,
              {
                  name: alt.name,
                  realm: alt.realm,
...
async function handleSetCommand(client, interaction, lang) {
    ...
    // Check permissions first, before deferring
        ...
        const characterData = await CharacterUtils.verifyCharacter(
            charNameFormatted,
            realm
        );
        if (!characterData) {
            return interaction.reply({
                content: LanguageManager.getText(
                    'commands.setchar.char_not_exist',
                    lang,
                    {
        ...
        const existingOwner = await checkExistingOwner(
            client,
            charNameFormatted,
            realm
        );
        ...
        if (existingOwner) {
            const shouldContinue = await handleExistingOwner(
                interaction,
                existingOwner,
                charNameFormatted,
                lang
        ...
        // Process the command
        await updateCharacterOwnership(
            client,
            interaction,
            user,
            charNameFormatted,
...
async function handleNameCommand(client, interaction, lang) {
    ...
    if (!member) {
        return interaction.editReply({
            content: LanguageManager.getText(
                'commands.global_strings.user_not_found',
                lang
            ),
            flags: [MessageFlags.Ephemeral],
        });
    ...
    try {
        ...
        if (!characterData) {
            return await interaction.editReply({
                content: LanguageManager.getText(
                    'commands.charinfo.char_not_exist',
                    lang,
                    {
                        character: charNameFormatted,
                    }
        ...
        const { summary, armory } = await CharacterUtils.fetchCharacterData(
            charNameFormatted,
            realm
        );
        await processAndDisplayCharacterInfo(
            client,
            interaction,
            summary,
            armory,
...
async function processAndDisplayCharacterInfo(
    client,
    interaction,
    summary,
    armoryBody,
    ...
    const totalGearScore = CharacterUtils.calculateGearScore(
        summary.equipment,
        itemsMap
    );
    ...
    const { missingGems, missingEnchants } = await processArmoryData(
        $,
        character,
        itemsDB
    );
...
function processArmoryData($, character, itemsDB) {
    ...
    $('.item-model a').each(function () {
        ...
        const gemsArray = params['gems'] ? params['gems'].split(':') : [];
        const amount = gemsArray.filter((x) => x !== '0').length;
    ...
    if (shouldCheckMissing) {
        ...
        items.forEach((item) => {
          ...
          (foundItem.type === 'Gloves' || foundItem.type === 'Bracer')
                ...
                // For rings, check if actual gems meet or exceed the expected amount
                if (foundItem.type === 'Ring #1' || foundItem.type === 'Ring #2') {
                    if (foundItem.gems < item.gems) {
                        missingGems.push(foundItem.type);
                    }
                } else if (foundItem.type === 'Belt') {
...
function setCharacterAppearance(character) {
    ...
    character.faction === 'Alliance'
        ? config.base.iconAlliance
        : config.base.iconHorde;
    ...
    const portraits = {
        ...
        'Night Elf': {
            Female: config.base.nightElfFemale,
            Male: config.base.nightElfMale,
        },
        ...
        'Blood Elf': {
            Female: config.base.bloodElfFemale,
            Male: config.base.bloodElfMale,
        },
        Draenei: {
            Female: config.base.draeneiFemale,
            Male: config.base.draeneiMale,
        },
    };
    character.portrait =
    portraits[character.race]?.[character.gender] ||
    config.base.defaultPortrait;
}

async function createCharacterEmbed(
    client,
    character,
    charNameFormatted,
    realm,
    ...
    // Add owner info
    const ownerInfo = await CharacterUtils.findCharacterOwner(
        client,
        character.name,
        realm
    );
    if (ownerInfo) {
        embed.addFields({
            name: LanguageManager.getText(
                'commands.charinfo.embed.fields.belongs_to',
                lang
            ),
...
async function processSetCommand(
    client,
    interaction,
    user,
    charNameFormatted,
...
async function checkExistingOwner(client, charNameFormatted, realm) {
    const allUserChars = await client
        .getDatabaseHandler()
        .find('userCharacters', {});
...
async function handleExistingOwner(
    interaction,
    existingOwner,
    charNameFormatted,
    lang
    ...
    const isDeveloper = config.users.developers.includes(interaction.user.id);
    const baseMessage = LanguageManager.getText(
        'commands.setchar.char_already_assigned',
        lang,
        {
            character: charNameFormatted,
...
async function updateCharacterOwnership(
    client,
    interaction,
    user,
    charNameFormatted,
    ...
    await client
        .getDatabaseHandler()
        .updateOne('userCharacters', { userId: user.id }, userData, {
            upsert: true,
        });

    const responseContent = createResponseContent(
        isMain,
        userData,
        charNameFormatted,
        realm,
...
function addBasicFields(embed, character, lang) {
    const baseFields = [
        ...
        character.honorableKills > 0 && [
            'honorable_kills',
            character.honorableKills.toString(),
        ],
        character.achievementPoints > 0 && [
            'achievement_points',
            character.achievementPoints.toString(),
        ],
        character.totalGearScore > 0 &&
      character.level > 60 && ['gearscore', character.totalGearScore],
    ].filter(Boolean);

    baseFields.forEach(([key, value]) => {
        embed.addFields({
            name: LanguageManager.getText(
                `commands.charinfo.embed.fields.${key}`,
                lang
            ),
            value: value,
            inline: true,
...
async function removeExistingOwnership(
    client,
    existingOwner,
    charNameFormatted,
    realm
) {
    const prevUserData = await client
        .getDatabaseHandler()
        .findOne('userCharacters', { userId: existingOwner.userId });
    if (existingOwner.isMain) {
        prevUserData.main = null;
    } else {
        prevUserData.alts = prevUserData.alts.filter(
            (alt) =>
                !(
                    alt.name.toLowerCase() === charNameFormatted.toLowerCase() &&
          alt.realm === realm
